...
export default function ItemRow({ item, orderSystem, featuredMode }: Props) {
  ...
  return (
    <motion.div
      layout
      className={`relative group flex flex-col gap-2 p-4 sm:p-5 bg-(--bg-card)/40 backdrop-blur-md rounded-4xl border border-(--border-color) transition-all duration-500 ${unavailable ? "opacity-60 grayscale-[0.8]" : "hover:border-primary/30 hover:shadow-xl hover:bg-(--bg-card)"
    ...
    >
      ...
      {itemIngredients && (
        <div className="pt-1.5 border-t border-(--border-color)/30">
          <p className="text-xs sm:text-sm text-(--text-muted) font-medium leading-relaxed opacity-80 text-right">
            {itemIngredients}
          </p>
        </div>
      )}
